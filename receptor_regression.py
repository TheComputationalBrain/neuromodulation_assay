...
for y_name in y_names:
    ...
    for sub in subjects:
        print(f"--- dominance analysis subject {sub} ----")
        y_data = np.load(os.path.join(home_dir[DATA_ACCESS],DB_NAME,MASK_NAME,'first_level',f'sub-{sub:02d}_{y_name}_{MASK_NAME}_effect_size_map.pickle'), allow_pickle=True).flatten()
        data_reg = create_dataframe(y_data, y_name, X_data, receptor_names) 
        data_reg = data_reg.dropna()
        dominance_regression=Dominance(data=data_reg,target=y_name,top_k=19) 
        ...
        results_df = pd.concat([results_df, results], ignore_index=True)
    #save data 
    fname = f'{y_name}_{MASK_NAME}_dominance_results_bysubject.pickle'
    results_df.to_csv(os.path.join(output_dir, fname), index=False)  
...
for y_name in y_names:
    results_df = pd.DataFrame(columns=[receptor_names])
    for sub in subjects:
        print(f"--- dominance analysis 2 subject {sub} ----")
        y_data = np.load(os.path.join(home_dir[DATA_ACCESS],DB_NAME,MASK_NAME,'first_level',f'sub-{sub:02d}_{y_name}_{MASK_NAME}_effect_size_map.pickle'), allow_pickle=True).flatten()
        non_nan_indices = ~np.isnan(y_data)
        X = X_data[non_nan_indices,:]
        y = y_data[non_nan_indices]
    ...
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    np.save(os.path.join(output_dir, f'{y_name}_{MASK_NAME}_dominance_bysubject.pickle'), results_df) 
