...
#sklearn regression
columns = np.concatenate((receptor_names_core, np.array(["R2", "adjusted_R2", "BIC"])))
...
for y_name in y_names:
    ...
    fname = f'{y_name}_{MASK_NAME}_regression_results_bysubject_core.csv'
...
#         results_df = pd.concat([results_df, results], ignore_index=True)
#     #save data 
#     fname = f'{y_name}__{ATLAS_MASK}_dominance_results_bysubject.pickle'
#     results_df.to_csv(os.path.join(output_dir, fname), index=False)  
...
#         os.makedirs(fname)
#     np.save(os.path.join(output_dir, f'{y_name}__{ATLAS_MASK}_dominance2_results_bysubject.pickle'), results_df) 