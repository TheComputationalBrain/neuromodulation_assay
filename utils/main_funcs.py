...
from nilearn import image, datasets, masking, surface, plotting
...
def load_receptor_array(paths, rec, on_surface):
    if on_surface:
        receptor_data =zscore(np.load(os.path.join(paths.receptor_dir, rec.source,f'receptor_density_surf.pickle'), allow_pickle=True))
    else:
        receptor_data = zscore(np.load(os.path.join(paths.receptor_dir, rec.source,f'receptor_density.pickle'), allow_pickle=True))

    return receptor_data

def load_surface_effect_maps_for_cv(subjects, task, latent_var, beta_dir, add_info):
    ...
    if task == 'lanA':
        ...
        fmri_files_all = sorted(glob.glob(os.path.join(beta_dir,f'sub-*_{latent_var}_effect_size_map{add_info}.nii.gz')))
        fmri_files = []
...
def nii_to_cortical_voxel_array(
    ...
    effect_img = image.resample_to_img(
    effect_img,
    mni_template,
    ...
    # Load Schaefer atlas qnd surpess nilearn fetch messages 
